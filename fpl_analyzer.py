        ...
        try:
                ...
                <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #161b22; border-radius: 16px; margin-bottom: 24px; border: 1px solid #30363d;">
                    <tr>
                        <td style="padding: 24px;">
                            <div style="font-size: 1.3em; font-weight: 700; color: #00ff87;">👋 Hei {subscriber_name}! Her er din personlige FPL-analyse</div>
                        </td>
                    </tr>
                </table>'''
            ...
            if deadline_str:
                ...
                if tid_igjen.total_seconds() > 0:
                    ...
                    warning_html = ""
                    if dager == 0 and timer < 6:
                        warning_html = '<td style="vertical-align: middle;"><div style="background: rgba(255,107,107,0.15); border: 1px solid #ff6b6b; color: #ff6b6b; padding: 8px 14px; border-radius: 8px; font-weight: 600;">⚠️ Bare noen timer igjen!</div></td>'
                    elif deadline_date == today_date:
                        warning_html = '<td style="vertical-align: middle;"><div style="background: rgba(255,107,107,0.15); border: 1px solid #ff6b6b; color: #ff6b6b; padding: 8px 14px; border-radius: 8px; font-weight: 600;">⚠️ Deadline er i dag!</div></td>'
                    elif deadline_date == tomorrow_date:
                        warning_html = '<td style="vertical-align: middle;"><div style="background: rgba(255,165,2,0.15); border: 1px solid #ffa502; color: #ffa502; padding: 8px 14px; border-radius: 8px; font-weight: 600;">📅 Deadline er i morgen!</div></td>'
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #161b22; border-radius: 16px; margin-bottom: 24px; border: 1px solid #30363d;">
                        <tr>
                            <td style="padding: 24px;">
                                <!-- Greeting -->
                                <div style="font-size: 1.3em; font-weight: 700; color: #00ff87; margin-bottom: 16px;">👋 Hei {subscriber_name}!</div>
                                
                                <!-- Deadline row -->
                                <table width="100%" cellpadding="0" cellspacing="0" style="border-top: 1px solid #30363d; padding-top: 16px;">
                                    <tr>
                                            ...
                                            <table cellpadding="0" cellspacing="0">
                                                <tr>
                                                    ...
                                                    <td style="vertical-align: middle;">
                                                        <div style="font-size: 1rem; font-weight: 600; color: #8b949e;">Gameweek {neste_gw.get('id', '?')} Deadline</div>
                                                        <div style="font-size: 1.5rem; font-weight: 700; color: #00ff87;">{dager}d {timer}t {minutter}m</div>
                                                        <div style="font-size: 0.85rem; color: #8b949e;">{deadline.strftime('%A %d. %B %H:%M')}</div>
                                                    </td>
                                                </tr>
                                            </table>
                                        </td>
    ...
    def _get_value_for_money_html(self):
        ...
        try:
            ...
            for _, row in top_value.iterrows():
                ...
                # Posisjons-farger (matching new design)
                pos_colors = {
                    'GKP': 'background-color: #ff9f43; color: #000;',
                    'DEF': 'background-color: #2ed573; color: #000;',
                    'MID': 'background-color: #1e90ff; color: #fff;',
                    'FWD': 'background-color: #ff4757; color: #fff;'
                }
                pos_style = pos_colors.get(row['pos'], 'background-color: #0d1117; color: #e6edf3;')
                
                rows_html += f'''
                <tr>
                    <td style="padding: 12px 10px; border-bottom: 1px solid #30363d;"><span style="{pos_style} padding: 4px 8px; border-radius: 4px; font-size: 0.75rem; font-weight: 700;">{row['pos']}</span></td>
                    <td style="padding: 12px 10px; border-bottom: 1px solid #30363d; font-weight: 600; color: #e6edf3;">{row['web_name']}</td>
                    <td style="padding: 12px 10px; border-bottom: 1px solid #30363d;"><span style="background-color: #0d1117; color: #8b949e; padding: 4px 8px; border-radius: 4px; font-size: 0.75rem;">{team_name}</span></td>
                    <td style="padding: 12px 10px; border-bottom: 1px solid #30363d; color: #8b949e;">£{row['pris_mill']:.1f}m</td>
                    <td style="padding: 12px 10px; border-bottom: 1px solid #30363d;"><span style="background: rgba(0,255,135,0.15); color: #00ff87; padding: 4px 10px; border-radius: 6px; font-weight: 700;">{row['xPts_adjusted']:.2f}</span></td>
                    <td style="padding: 12px 10px; border-bottom: 1px solid #30363d;"><span style="background: rgba(255,215,0,0.15); color: #ffd700; padding: 4px 10px; border-radius: 6px; font-weight: 700;">{row['value']:.2f}</span></td>
                </tr>'''
            
            html = f'''
        <table width="100%" cellpadding="0" cellspacing="0" style="background: linear-gradient(135deg, rgba(0,255,135,0.1) 0%, rgba(0,217,114,0.05) 100%); border-radius: 16px; margin-bottom: 24px; border: 1px solid rgba(0,255,135,0.3);">
            <tr>
                <td style="padding: 24px;">
                    <div style="margin-bottom: 20px;">
                        <span style="font-size: 1.8em; margin-right: 12px;">💰</span>
                        <span style="font-size: 1.3rem; font-weight: 700; color: #e6edf3;">Beste Value for Money</span>
                        <div style="color: #8b949e; font-size: 0.9rem; margin-top: 4px; margin-left: 48px;">Høyest xPts per million (xPts/£m) per posisjon</div>
                    </div>
                    
                    <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse: collapse;">
                        <thead>
                            <tr>
                                <th style="padding: 12px 10px; text-align: left; border-bottom: 1px solid #30363d; color: #8b949e; font-weight: 600; font-size: 0.8rem; text-transform: uppercase;">Pos</th>
                                <th style="padding: 12px 10px; text-align: left; border-bottom: 1px solid #30363d; color: #8b949e; font-weight: 600; font-size: 0.8rem; text-transform: uppercase;">Spiller</th>
                                <th style="padding: 12px 10px; text-align: left; border-bottom: 1px solid #30363d; color: #8b949e; font-weight: 600; font-size: 0.8rem; text-transform: uppercase;">Lag</th>
                                <th style="padding: 12px 10px; text-align: left; border-bottom: 1px solid #30363d; color: #8b949e; font-weight: 600; font-size: 0.8rem; text-transform: uppercase;">Pris</th>
                                <th style="padding: 12px 10px; text-align: left; border-bottom: 1px solid #30363d; color: #8b949e; font-weight: 600; font-size: 0.8rem; text-transform: uppercase;">xPts</th>
                                <th style="padding: 12px 10px; text-align: left; border-bottom: 1px solid #30363d; color: #8b949e; font-weight: 600; font-size: 0.8rem; text-transform: uppercase;">xPts/£m</th>
                            </tr>
                        </thead>
                        <tbody>
                            {rows_html}
...
<head>
    ...
    <style>
        ...
        * {{
        ...
        tr:nth-child(2) td {{ background: rgba(192, 192, 192, 0.08); }}
        tr:nth-child(3) td {{ background: rgba(205, 127, 50, 0.08); }}
        .medal {{
            display: inline-flex;
            align-items: center;
        ...
        .pos-gkp {{ background: #ff9f43; color: #000; }}
        .pos-def {{ background: #2ed573; color: #000; }}
        .pos-mid {{ background: #1e90ff; color: #fff; }}
        .pos-fwd {{ background: #ff4757; color: #fff; }}
        .player-name {{
            font-weight: 600;
            color: var(--text);
        }}
        ...
        .dream-team-section .section-title {{ color: var(--gold); }}
        .dream-team-section .section-desc {{ color: var(--text-muted); }}
        .dream-team-section th {{ color: var(--gold) !important; }}
    </style>
</head>
<body>
    <div class="container">
